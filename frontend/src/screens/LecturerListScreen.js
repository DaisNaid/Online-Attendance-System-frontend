...
export default function LecturerListScreen() {
  ...
  const deleteHandler = async (lecturer) => {
    if (window.confirm('Are you sure to delete?')) {
      try {
        dispatch({ type: 'DELETE_REQUEST' });
        await axios.delete(`/api/users/${lecturer._id}`, {
          headers: { Authorization: `Bearer ${userInfo.token}` },
        });
                ...
                {users.map((lecturer) => (
                  <tr key={lecturer._id}>
                    ...
                    <td>
                      {course.map((x) => (
                        <div key={x._id}>
                          {lecturer._id === x.lecturer ? x.title : <div></div>}
                        </div>
                      ))}
                    </td>
                    <td>{lecturer.createdAt.substring(0, 10)}</td>
                    <td>
                      <Button
                        type="button"
                        variant="light"
                        onClick={() => navigate(`/lecturer/${lecturer._id}`)}
                      >
                        Edit
                      </Button>
                      &nbsp;
                      <Button
                        type="button"
                        variant="light"
                        onClick={() => deleteHandler(lecturer)}
                      >
                        Delete
                      </Button>
                    </td>
